...
function buildAction({ icon, text }: Action, idx: number): any {
...
function buildActionsMenu(
    actions: Action[],
    handler: (param: ClickParam) => void,
): any {
    if (actions == null || actions.length === 0) {
        return null;
    }

    return <Menu onClick={handler}>{map(actions, buildAction)}</Menu>;
}
...
export default class ActionPanel extends React.Component<Props, State> {
    constructor(props: Props) {
        ...
        this.state = {
            actionsMenu: buildActionsMenu(
                props.actions,
                this.__handleActionClick,
            ),
        };
    }
    ...
    public render(): any {
        ...
        return (
            <Dropdown overlay={actionsMenu}>
                <Button className={css.btn}>{this.__renderIcon()}</Button>
            </Dropdown>
        );
    }
